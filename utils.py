...
def create_prune_mask(model, percent):
    """
    Compute masks for model parameters so as to prune the "percent" smallest
    parameters would be reset after multiplying masks by corresponding
    parameters
    """
    all_weights = []
...
def diff_init_weight_prune_mask(model, pruning_perc):
    ...
    changed parameters as compared to initial state of parameters
    ...
    threshold = np.percentile(
        abs(np.array(all_init_weights)) - abs(np.array(all_weights)), pruning_perc)
...
def train(model, loss_fn, optimizer, epochs, train_loader, val_loader):
    ...
    for epoch in range(epochs):
        ...
        print("Train epoch: {}, Val accuracy: {}".format(epoch, val_acc))
...
def train_one_shot(model, loss_fn, optimizer,
    ...
    """
    Implements one shot pruning described in the original paper.
    """
    history = {}
    print('Preparing: Train before prune...')
    history['train-pre-prune'] = train(model, loss_fn, optimizer,
                                       prepare_epochs,
                                       train_loader, val_loader)

    print('\nPruning...')
    masks = create_prune_mask(model, prune_percent)
    ...
    history['train-post-prune'] = train(model, loss_fn, optimizer,
                                        prune_epochs,
                                        train_loader, val_loader)
